...
n = ['1', '2', '3', '4']
choice = input('Choose formation ([1] for 4-3-3 (default) or 2 for 4-3-2-1 or 3 for 4-4-2 or 4 for 5-3-2): ')
if choice not in n:
    choice = 1
choice = int(choice)
...
C3 = M * Constraint((sum(x[n] for n in range(2, 7)) - 1)**2,
                    label='x2+x3+x4+x5+x6=1')
...
C6 = M * Constraint((sum(x[n] for n in range(38, 43)) - 2)**2,
                    label='x38+x39+x40+x41+x42=2')
...
C10 = M * Constraint((sum(x[n] for n in range(11, 16)) - 1)**2,
                     label='x11+x12+x13+x14+x15=1')
# C11 = M * Constraint((sum(x[n] for n in range(28, 33)) - 3)**2,
#                      label='x28+x29+x30+x31+x32=3')
C11 = M * Constraint((sum(x[n] for n in range(28, 33)) - 2)**2,
                     label='x28+x29+x30+x31+x32=2')
...
if choice == 1:
    ...
    # Hamiltonian for formation 4-4-2
    H = -1 * HZ + C1 + C2 + C3 + C4 + C5 + C6 + C8 + C9 + C10 + \
        I1 + I2 + I3 + I4 + I5 + I6 + I7 + I8 + I9 + I10
else:
    # Hamiltonian for formation 5-3-2
    H = -1 * HZ + C1 + C2 + C3 + C4 + C5 + C6 + C8 + C9 + C10 + C11 + \
        I1 + I2 + I3 + I4 + I5 + I6 + I7 + I8 + I9 + I10
